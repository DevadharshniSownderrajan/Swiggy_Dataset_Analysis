...
SELECT ORDERID,YEAR(ORDERDATE) AS YEARS
FROM ORDERS;

#98.	Display the order month for every order. 
SELECT ORDERID,MONTH(ORDERDATE) AS MONTHS
FROM ORDERS;
...
SELECT ORDERID,WEEKDAY(ORDERDATE) AS WEEKDAY_
FROM ORDERS;
...
SELECT ORDERID,ORDERDATE,QUARTER(ORDERDATE) AS QUAETER_OF_EVERY_ORDER
FROM ORDERS;
